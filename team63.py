...
class Team63():
	...
	def move(self, board, old_move, flag):
		...
		for cell in cells:
			...
			moveVal = self.minimax(board, 0, 1, cell, -10000, 10000)
	...
	def heuristic(self, board, isMax):
		...
		# if board.big_boards_status[0][1][1] == 'x':
		# 	print self.sbs_score
		# 	ch =raw_input("sdf")
		index1 = 3;
		for k in range(2): #Big boards 1 and 2
			...
			for l in range(3): # Going through 3 small boards in one big board
				...
				for i in range(index1, index1+3):
					...
					number_of_o = 0;
					for j in range(index2, index2+3):
						if ( board.big_boards_status[k][j][i] == self.player1):
							number_of_x += 1;
						if ( board.big_boards_status[k][j][i] == self.player2):
							number_of_o += 1;
					self.calculate_sbScore(number_of_x, number_of_o)
				#checking if smallboard is won
				# ans = self.check_win();
				...
				#applying our formula for calculating score for this smallboard
					...
					# if board.big_boards_status[0][0][3] == 'x' and k==0 and l==1:
					# 	print self.sbs_score
					# 	print total
					
					# self.sbs_score = self.sbs_score + total;
		...
		for k in range(2): #Big boards 1 and 2
			...
			for l in range(3): # Going through 3 small boards in one big board
				...
				#applying our formula for calculating score for this smallboard
					...
					# 	total *= -1
					self.sbs_score += total;
					# print("Total Myscore- " + str(total_myscore))
					# print("Total Oppscore- " + str(total_oppscore))
